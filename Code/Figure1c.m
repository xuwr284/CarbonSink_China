clear
clc
folder = ['I:\CarbonF\CarbonSink_China\'];
...
p0 = [agb_int(agb_int(:,2)==ID,16) 3 c_init];
...
f = figure('visible','on');
set(f,'Units','inches','Position',[0 0 8 5]);   % [left bottom width height]
 hold on
...
writematrix(curve,[folder,folderdir,'\REF_natural_curve.csv']);
...
p11(:,3)=0:xmax;
curve(igrid,1)=ID;curve(igrid,2:4)=p;curve(igrid,5)=rsq;curve(igrid,6)=rmse;
writematrix(curve,[folder,folderdir,'\AFF_curve.csv']);
...
xlabel('Ages (years since disturbance/afforestation)')
ylabel('AGB (Mg/ha)')
...
h1 = plot(nan,nan,'-','Color','k','LineWidth',1);
h2 = patch(nan,nan,'k','FaceAlpha',0.3,'EdgeColor','none');
h3 = plot(nan,nan,'ko','MarkerFaceColor','none','MarkerSize',5,'LineStyle','none');
h4 = plot(nan,nan,'.','Color','k','MarkerSize',20);
h5 = plot(nan,nan,'k--','LineWidth',1);
...
%legend( 'Fitted curve','Uncertainty range','data points','AGBmax','AGB_eq','Location', 'east' );
%text(20,ymax*0.25,sprintf('R^2 = %3.2g\nRMSE = %2.4g',rsq,rmse),'FontSize',9)
%legend( 'Fitted curve reforestation (REF)','REF Uncertainty range','REF points','AGBmax','REF AGB_eq','Fitted curve afforestation (AFF)','AFF Uncertainty range','AFF points','AFF AGB_eq', 'Location', 'southeast' ); % 'GlobBiomass2010',
%lgd = findobj('type', 'legend');
%delete(lgd)
hold off

exportgraphics(f,[folder,folderdir,'\curve_',num2str(ID),'.jpg'],Resolution=300)
close(f)
